...
EllipseBoard::EllipseBoard(int width, int height, int columns, int rows, bool randomSquare)
{
...
void EllipseBoard::FillFullEllipse(int row, int col, int wid, int hgt)
{
    ...
    bool  outer = m_random ? ((rand() % 2) == 0 ? true : false) : false;
    bool  inner = !outer;
	...
	for (auto y = 0; y < hgt; y++)
	{
		...
		// In Ellipse
        for (auto x = from; x < to; x++)		SetAt(thisrow, int(col + x), inner);
		// After Ellipse
        for (auto x = to; x < wid; x++)			SetAt(thisrow, int(col + x), outer);
	}
}
