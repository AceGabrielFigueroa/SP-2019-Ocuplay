...
BooleanBitmap::BooleanBitmap(BooleanBitmap* chb1, BooleanBitmap* chb2, bool _xor )
{
	...
	for (auto row = 0; row < m_heightPixels; row++) {
		...
		{
            bool pixelValue = chb1->GetAt(row, col);
            if (_xor )
	...
	int fromCol, int fromRow, int toCol, int toRow, int size)
{
	...
	for (auto col = 0; col < size; col++)
	{
		...
		for (auto row = 0; row < size; row++) 
		{
            ...
            bool fromVal = fromBM->GetAt(fromRow_r, size - 1 - fromCol_c);
	...
	int fromCol, int fromRow, int toCol, int toRow, int wid, int hgt)
{
	...
	for (auto col = 0; col < wid; col++)
	{
		...
		for (auto row = 0; row < hgt; row++)
		{
            //bool v = fromBM->GetAt(wid - 1 - fromCol_c, fromRow + row);
			//SetAt(toCol_c, toRow + row, v );
            //bool v = fromBM->GetAt(fromCol_c, fromRow + row);
			//SetAt(toCol_c, toRow + row, v);
	...
	int fromCol, int fromRow, int toCol, int toRow, int wid, int hgt)
{
	...
	for (auto col = 0; col < wid; col++)
	{
		...
		for (auto row = 0; row < hgt; row++)
		{
            //bool v = fromBM->GetAt(fromCol_c, hgt - 1 - fromRow - row);
			//SetAt(toCol_c, toRow + row, v );
            bool v = fromBM->GetAt( fromRow + row, fromCol_c);
...
// Prints out the board with ascii '*' and '.'
...
//	sprintf_s(bufr, "\nRows %d  Cols %2d\n", m_heightPixels, m_widthPixels);
	for (auto row = 0; row < m_heightPixels; row++) {
        ...
        for (auto col = 0; col < m_widthPixels; col++) {
            ...
            buffer.append((GetAt(row, col) ? '*' : '.'));

            /*
            CH(GetAt(row, col) ? '*' : '.'); qDebug().nospace() << bufr;
            */
		}
        //NL(); qDebug().nospace() << bufr;
        //qDebug() << endl;
        qDebug() << buffer;
	}
//	CH('-');
